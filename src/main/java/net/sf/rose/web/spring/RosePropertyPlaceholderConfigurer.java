...
* @version 创建时间：2017年4月7日 下午11:51:53 
* 类说明 
*/
public class RosePropertyPlaceholderConfigurer extends PropertyPlaceholderConfigurer {
	...
	private void fillProperties(Properties props, EncodedResource resource, PropertiesPersister persister)
		...
		try {
			...
			if (filename != null && filename.startsWith("system!")) {
				File path = BootStart.getInitialPath();
				filename = filename.substring(7, filename.length());
				filename = StringUtils.replace(filename, "projectName!", BootStart.getProjectName() + "/");
				stream = new FileInputStream(new File(path, filename));
				persister.load(props, stream);
			}
			else if (filename != null && filename.endsWith(XML_FILE_EXTENSION)) {
			 ...
			 BootStart.setConfigProperties(props);
		}
		finally {
			if (stream != null) {
				stream.close();
